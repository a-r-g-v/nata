...
class ServiceRecord(Base):
    ...
    lb = relationship(
        'LbRecord', lazy="joined", uselist=False, backref="service")
    apps = relationship(
        'AppRecord', lazy="joined", uselist=True, backref="service")
...
class LbRecord(Base):
    ...
    serviceno = Column(
        Integer, ForeignKey('service.no'), unique=True, nullable=False)
